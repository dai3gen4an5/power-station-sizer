...
export function ScExample4h() {
  ...
  return (
    <div>
      <h2 className="h2">200&nbsp;W, 4-hour example</h2>
      <p className="mt-3 text-ink/75">
        A small {WATTS.toLocaleString("en-US")}&nbsp;W slow cooker run for {HOURS} hours. Example
        figures &mdash; not every small cooker is 200&nbsp;W, so use your own label wattage and cook
        time.
      </p>
      <div className="mt-4 card card-pad">
        ...
        <p className="mt-3 text-sm text-ink/75">
          ...
          <span className="font-mono">{WATTS.toLocaleString("en-US")} W</span>, which every listed
          unit clears, so the recommendation stays in the{" "}
          <span className="font-semibold">{finalLabel}</span> range on capacity grounds alone. Even a
          low-wattage cooker on a half-day cook needs a mid-to-large power station &mdash; the hours
          do the work. The calculator&apos;s own rounded size figure ({sizeLabel}) tracks the cook
          energy, not the catalogue range.
        </p>
      </div>
    </div>
  );
